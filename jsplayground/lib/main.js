...
function newPokestop() {
  ...
  var new_name = document.getElementById('new_stop_name'); // 2. Grab the input url

  var new_url = document.getElementById('new_stop_url'); // 2.a Make sure there is a protocol at the beginning of the url
  ...
  if (new_url.value.substring(0, 7) == "http://" || new_url.value.substring(0, 8) == "https://") {
    // 3. Grab the UL
    var pokestops = document.getElementById('pokestops'); // 4. Create a new list item

    var new_li = document.createElement("li"); // 5. Create a new anchor tag

    var new_a = document.createElement("a"); // 6. Create a new text node (something that would display on the screen)

    var new_text = document.createTextNode(new_name.value); // 7. Add the text to the anchor (this is the text between <a> and </a>)

    new_a.appendChild(new_text); // 8. Add the href attribute and a blank target to the anchor

    new_a.href = new_url.value;
    new_a.target = "_blank"; // 9. Add the anchor to the li

    new_li.appendChild(new_a); // 10. Prepend li before first li that's already there

    var first_li = document.querySelector("#pokestops li:first-of-type");
    pokestops.insertBefore(new_li, first_li); // This method was suggested here:
    ...
    alert.classList.add('alert-success'); // 12. Blank out the name and the url
...
const newGym = () => {
  ...
  const valid_match = /^https?:\/\//i; // console.log("1", new_item);
  ...
  if (new_url.value.search(valid_match) === 0) {
    ...
    new_a.target = "_blank";
    new_a.href = new_url.value;
    new_item.appendChild(new_a); // console.log("2", new_item);
    // *** magic ***
